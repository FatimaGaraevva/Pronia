...
namespace Pronia.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateProductVM productVM)
        {
            ...
            if (!result)
            {
                ModelState.AddModelError(nameof(CreateProductVM.CategoryId), "category dose not exsist");
                return View(productVM);
            }
            if (!productVM.MainPhoto.ValidateType("image/"))
            {
                ModelState.AddModelError(nameof(CreateProductVM.MainPhoto), "File type is inccorect");
                return View(productVM);
            }
        ...
        public async Task<IActionResult> Update(int? id,UpdateProductVM productVM)
        {
            ...
            if (productVM.MainPhoto is not null)
            {
                if (!productVM.MainPhoto.ValidateType("image/"))
                {
                    ModelState.AddModelError(nameof(UpdateProductVM.MainPhoto), "File type is inccorect");
                    return View(productVM);
                }
                if (!productVM.MainPhoto.ValidateSize(FileSize.KB, 500))
                {
            ...
            if (nameResult)
            {
                ModelState.AddModelError(nameof(UpdateProductVM.Name), "product alerady exists");
                return View(productVM);
            }
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            foreach ( ProductImage protImage in product.ProductImages)
            {
                protImage.Image.DeleteFile(_env.WebRootPath, "assets", "images", "website-images");
